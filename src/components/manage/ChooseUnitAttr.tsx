...
export interface ChooseUnitAttrValue {
  primary_id: number;
  primary_name: string;
  secondary_id: number;
  secondary_name: string;
}
interface ChooseUnitAttrProps {
  value?: ChooseUnitAttrValue[];
  options: Options[];
  onChange?: (v: ChooseUnitAttrValue[]) => void;
}
const ChooseUnitAttr = ({
  value = [],
  options = [],
  onChange = () => {},
}: ChooseUnitAttrProps) => {
        ...
        <div key={index} className={css({ display: "block" })}>
          ...
          >
            <Cascader
              ...
              onChange={(v) => {
                if (!v) return;
                let p_item = options.find((o) => o.value == v[0]);
