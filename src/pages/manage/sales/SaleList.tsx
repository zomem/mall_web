...
const SaleList = () => {
  ...
  useEffect(() => {
    if (showUid > 0) {
      get("/manage/sales/sale_sub/list/" + showUid + "/" + 1 + "/" + 1000).then(
        (res) => {
  ...
  function change_status(id: number, status: NormalStatus) {
    put("/manage/sales/sale_user/status", { id, status }).then((res) => {
      if (res.status == 1) {
  ...
  function delete_confirm(id: number) {
    put("/manage/sales/sale_user/del", { id }).then((res) => {
      if (res.status == 1) {
        message.success(res.message);
        setR((p) => p + 1);
      } else {
