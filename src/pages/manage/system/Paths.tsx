...
function validatePrimeNumber(name: string): {
...
function Paths() {
  ...
  useEffect(() => {
    get("/manage/system/paths/list").then((res) => {
      setInfo({
        loading: false,
        list: res.objects,
      });
      let temp: any = [];
      for (let item in res.objects) {
        temp.push({
          value: res.objects[item].name,
          path: res.objects[item].path,
          icon: res.objects[item].icon_name,
        });
      }
      setNameList(temp);
    });
  ...
  useEffect(() => {
    if (expandPath) {
      get("/manage/system/sub_paths/list/" + expandPath).then((res) => {
        setSubInfo((prev: any) => ({
          ...prev,
          loading: false,
          [`${expandPath}`]: res.objects,
        }));
      });
    }
  }, [refresh, expandPath]);
  ...
  const onFinish = (values: any) => {
    ...
    if (values.path[0] == "/" || values.path[values.path.length - 1] == "/") {
      return message.error("页面目录或路径，首尾不用写 “/”");
    }
    if (
      values.sub_path[0] == "/" ||
      values.sub_path[values.sub_path.length - 1] == "/"
    ) {
      return message.error("页面目录或路径，首尾不用写 “/”");
    }
    setLoading(true);
      ...
      },
      {
        ...
        render: (record: any) => (
          <Popconfirm
            title="确定删除该子模块吗？"
            onConfirm={() => {
              post("/manage/system/paths/delete", { id: record.id }).then(
                (res) => {
                  if (res.status === 1) {
                    message.success(res.message);
                    setRefresh((prev) => prev + 1);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "icon_name",
      width: 80,
      render: (item: any) => <ExcIcons name={item} />,
    },
    {
      title: "父模块名",
      dataIndex: "name",
  ...
  return (
    <>
      ...
      >
        <IconsList select={iconName} onClick={(name) => setIconName(name)} />
      </Modal>
    </>
  );
}
